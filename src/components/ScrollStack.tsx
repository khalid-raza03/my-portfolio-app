...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const useSmoothScroll = smoothScroll ?? !useWindowScroll;
  ...
  syncTouchLerp: 0.1  
    ...
    lenisRef.current = lenis;
    return lenis;
  }, [handleScroll, useWindowScroll, useSmoothScroll]);
  ...
  const innerClasses =
    `scroll-stack-inner  px-4 sm:px-8 lg:px-12 pb-[24vh] min-h-full ${innerClassName}`.trim();
    ...
    return (
      <div ref={rootRef} className={`scroll-stack-root relative w-full ${className}`.trim()}>
        <div className={innerClasses}>
          {children}
          <div className="scroll-stack-end w-full h-[60vh]" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div ref={rootRef} className={`scroll-stack-root relative w-full h-full ${className}`.trim()}>
      <div
        className="relative w-full h-full overflow-y-auto overflow-x-visible bg-transparent overscroll-y-contain"
        ref={scrollerRef}
        style={{
