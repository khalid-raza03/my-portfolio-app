...
export default function Navbar({ onMenuToggle, menuOpen }: NavbarProps) {
  ...
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    [
      "rounded-full px-4 py-2 text-sm font-medium transition max-[380px]:px-3 max-[380px]:py-1.5",
      isActive
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-4 py-4 sm:gap-0 sm:py-6 max-[380px]:px-3 max-[380px]:py-3">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="ml-1 inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#d6bea0] bg-[#f8ead8] text-zinc-700 transition hover:border-[#c9ae8d] hover:bg-[#f1dfca] dark:border-white/10 dark:bg-white/5 dark:text-zinc-200 dark:hover:bg-white/10 z-50 cursor-pointer relative"
          >
        ...
        <button
          ...
          className="sm:hidden inline-flex items-center justify-center h-10 w-10 rounded-full border border-[#d6bea0] bg-[#f8ead8] text-zinc-700 dark:border-white/10 dark:bg-white/5 dark:text-zinc-200 cursor-pointer transition-all duration-300"
        >
