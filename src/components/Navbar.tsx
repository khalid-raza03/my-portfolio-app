...
export default function Navbar({ onMenuToggle, menuOpen }: NavbarProps) {
  ...
  return (
    <header className="fixed w-full  top-0 z-40 border-b border-[#dcc4a8]/70 bg-[#fff8ee]/85 backdrop-blur-xl dark:border-white/10 dark:bg-zinc-950/75">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-4 py-4 sm:gap-0 sm:py-6 max-[380px]:px-3 max-[380px]:py-4">
        <div className="flex items-center gap-x-6  w-1/2 justify-start">
          ...
          >
            <MorphingText
              texts={["Khalid Raja", "Frontend Dev", "React Dev"]}
            />
          </Link>
        </div>
        ...
        <nav className="hidden sm:flex flex-wrap items-center gap-1 sm:justify-end sm:gap-2">
          ...
          <NavLink to="/details" className={({ isActive }) => `${linkClass({ isActive })} md:hidden`}>
            About
          </NavLink>
        </nav>
